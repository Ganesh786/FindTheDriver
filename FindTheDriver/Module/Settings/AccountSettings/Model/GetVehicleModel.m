...
-(void)getVehiclesAPICall:(NSString*)url completionBlock:(GetVehicleModelCompletionBlock)block{
    [[WebServiceInvoker sharedInstance]getToPath:[NSString stringWithFormat:@"%@%@%@",BASE_URL,GET_VEHICLES_URI,url] withParams:nil completion:^(BOOL success, NSString *message, NSDictionary *dataDict) {
        if (success) {
            //Update DB
        }
